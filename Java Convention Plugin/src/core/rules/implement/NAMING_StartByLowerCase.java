...
public class NAMING_StartByLowerCase extends RuleCheckerImplement {

	@Override
	public Warning check(Checker c, Token t, String[] args) {
		Warning warning = null;
		if (t.image.charAt(0) == Character.toLowerCase(t.image.charAt(0)))
			return warning;
		for (RuleRange rg : RuleRange.values()) {
			...
			Vector<Rule> rv = c.getRm().searchRule(
					RuleType.NAMING_StartByLowerCase, rg);
			for (Rule r : rv) {
				if (!r.active)
					break;
				...
				newName.insert(0, Character.toLowerCase(character));
